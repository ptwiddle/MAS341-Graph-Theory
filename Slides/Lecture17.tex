...
The \emph{chromatic index} $\chi^\prime(G)$ denotes the minimum number of colours needed to colour the \emph{edges} of $G$ so that any two edges that share a vertex have different colours.
    \end{definition}
  \begin{block}{What are the colours in the application?}
    \end{block}
...
\begin{block}{Examples:}
  ...
  \end{itemize}
\end{block}
$\Delta(G)$ is used to denote the maximum degree of a vertex.
...
\begin{frame}{How to find $\chi^\prime(G)$}
...
For a simple graph $\chi^\prime(G)=\Delta$ or $\Delta+1$
 ...
 \item Find a colouring with $\Delta+1$ colours 
   \end{itemize}
\end{block}
\begin{block}{What we mean by ' implicitly'} 
Vizing's Theorem tells us this last step is possible, but we'd still want to do it even if we know Vizing's Theorem.


   \end{block}
  \end{frame}

\begin{frame}{Another way to prove to get a lower bound on $\chi^\prime$}
Count how many edges could have the same colour.

  \begin{example}[$\chi^\prime(K_n)$ for $n$ odd]
