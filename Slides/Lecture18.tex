...
\begin{frame}{The two cases are themselves chromatic polynomials!}
  ...
  \begin{block}{Case 1: $v_1$ and $v_3$ are the same colour}
If they're the same colour, then we can make them same vertex...
        \end{block}
  
  \begin{block}{Case 2: $v_1$ and $v_3$ are different colours}
...
\begin{frame}{Generalizing the observation we just made}
  ...
  \begin{proof} Consider the colourings of $G$. In some of them $x$ and $y$ will be colours, and in others $x$ and $y$ will be the same colour.  The colourings in the first case are exactly the colourings of $G_{+xy}$, the colourings in the second are the colourings of $G_{x=y}$.
   \end{proof}
  ...
  \begin{tikzpicture}
    ...
    \begin{column}{.7\textwidth}
      \begin{block}{Three cases, but two are the same:}
        \begin{itemize}
        \item Case 1: 1,3 and 4 all have different colours
        \item Case 2: 1 and 3 have the same colour
        \item Case 3: 1 and 4 have the same colour
        \end{itemize}
...
The lemma as stated can be a bit awkward for induction:
\begin{itemize}
...
\begin{example} Re-do finding $P_{C_4}(k)$ and $P_{C_5}(k)$ using deletion-contraction.
